/*
 * @file ui_player.c
 * @brief UI screen for a music player interface
 *
 * This file implements the UI for a music player. It displays information
 * such as artist name, album/track title, and playback time. It includes
 * controls for play/pause, next/previous track, and volume adjustment
 * via an arc slider. An arc is also used to show playback progress.
 * The screen features entry animations for its elements and handles basic
 * interactions like returning to the main menu upon a long press event
 * from any of the control buttons or the volume arc.
 */
#include "lvgl.h"
#include <stdio.h>
#include "ui.h"
...
// LVGL object for the player UI page
static lv_obj_t *page;
// Callback function to execute when returning from this UI
static ret_cb_t return_callback;

/**
 * @brief Event callback for player control elements.
 *
 * This function handles events generated by interactive elements on the player UI,
 * such as the play, next, previous, mode buttons, and the volume arc.
 * - `LV_EVENT_FOCUSED`: Sets the LVGL group to editing mode. (Note: This is generally
 *   handled by the group itself or the parent object being focused).
 * - `LV_EVENT_KEY`: (Currently empty) Could be used for key-based interactions.
 * - `LV_EVENT_LONG_PRESSED`: Triggers the deletion of the player UI and
 *   calls the `return_callback` to go back to the previous screen. This is
 *   attached to all focusable control elements.
 *
 ...
 * It sets up the main page with a background image. Over this, it lays out:
 * - An arc for playback progress.
 * - An arc for volume control.
 * - Labels for artist name, album/track title, and current/total time.
 * - Buttons for play/pause, next track, previous track, and playback mode (e.g., loop).
 * Entry animations are applied to various elements for a dynamic appearance.
 * Interactive elements (buttons, volume arc) are added to the encoder group
 * for navigation, and event callbacks are registered for interaction (e.g.,
 * long press on any control to exit).
 ...
 * animations associated with this UI, and deletes the main page object along
 * with all its children (arcs, labels, buttons, image).
